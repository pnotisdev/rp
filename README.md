...
See [ROADMAP.md](ROADMAP.md) for the full history and for what's honestly still missing: proactive, unprompted messages from characters, a genuinely live turn-by-turn date mode instead of an ordinary chat with scoring bolted on, per-chat or per-world control over the dating-sim assist toggles (today they're one global setting), and mobile/responsive support.
